        ...
        const updateWebview = async () => {
            try {
                ...
                if (isBinary) {
                    ...
                    // Pass the TEMP FILE to Python
                    const result = await runBridgeJson(this.context.extensionPath, getCachedPythonExecutable(), ['ainb_rpc.py', '--file', document.tmpPath, '--command', commandString]);
                ...
                // Parse using the original file name so the adapter knows it's AINB
                const result = adapter.parse(document.uri.fsPath, rawText);
